...
| `tcp_syn_ack` | `5` | XSK RX + user-space TX | active | Redirect the original packet to XSK; user space builds TCP SYN-ACK and transmits through AF_XDP by default or AF_PACKET when egress is configured, with optional per-rule `response.params.tcp_seq` override |
